...
export interface Image extends Versioned {
	...
	extensions?: {
		/**
		 * whether to flip y direction
		 *
		 * used to fix coordinate orientation.
		 *
		 * @usage
		 *
		 * Most modeling software and 3D formats use different uv orientation than webgl.
		 * So flipY is set to `true` by default.
		 * Set this to `false` if image is GLTF texture or generated by framebuffer/canvas.
		 *
		 *
		 * 是否反转Y轴坐标
