...
float texture2DShadowLerp(sampler2DShadow depths, vec2 size, vec2 uv, float compare) {
    ...
    // 相对于四次取样略差，远好于直接用uv取，需要使用LinearFilter
    return texture(depths, vec3(centroidUV, compare));
...
float getShadow(sampler2D shadowMap, vec2 shadowMapSize, float shadowBias, float shadowRadius, vec4 shadowCoord) {
    ...
    if (frustumTest) {
        ...
        shadow = (texture2DShadowLerp(shadowMap, shadowMapSize, shadowCoord.xy + vec2(dx0, dy0), shadowCoord.z) +
