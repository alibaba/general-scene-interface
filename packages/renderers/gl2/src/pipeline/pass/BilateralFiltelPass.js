...
export default class BilateralFiltelPass extends Pass {
    constructor(conf, THREE) {
        ...
        this.filterXTarget = new THREE.WebGLRenderTarget(
            this.width, 
            this.height,
            {depthBuffer: false}
        );
		...
		this.filterX = new THREE.ShaderMaterial({
			...
			name: 'GL2::BilateralFiltelPass::Hor',
			defines: {
				DIRECTION_X: true,
			}
		})
		this.filterY = new THREE.ShaderMaterial({
			...
			fragmentShader: fs,
			uniforms: this.uniforms,
			name: 'GL2::BilateralFiltelPass::Ver',
			defines: {
				DIRECTION_Y: true,
			}
		})
