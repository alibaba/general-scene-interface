...
float fetchLinearDepth (vec2 depthUV) {
    ...
    return cameraCoef / (cameraFarPlusNear - texture2D( depthBuffer, depthUV ).x * cameraFarMinusNear);
}
...
float _trunc (float x) {
  return float(int(x));
}

float fmod (float x, float y) {
  return x - y * _trunc(x/y);
}

void main () {
  vec4 gBufferData = texture2D(normalBuffer, vUv);
//   float specularRoughness = gBufferData.a;
  ...
  vec3 decodedNormal = unpackRGBToNormal( gBufferData.rgb );
  ...
  vec4 originalPixel = texture2D(tex, vUv);
  vec4 ssrPixel = texture2D(tex, hitPixel);
  
  gl_FragColor = originalPixel * 0.5;
  gl_FragColor.rgb = ssrPixel.rgb;
//   gl_FragColor = ssrPixel;

//   gl_FragColor = vec4(1.0);
//   gl_FragColor.rg = vUv;
//   gl_FragColor.rg += 0.5 * hitPixel;
//   gl_FragColor += vec4(decodedDepth);
//   gl_FragColor.rgb += .5 * viewSpaceRayDirection;
//   gl_FragColor += vec4(intersect ? 1.0 : 0.0);
